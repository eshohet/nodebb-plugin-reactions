...
reactions.init = function (params, callback) {
        ...
        addPostReaction: function (socket, data, callback) {
            ...
            meta.settings.get('reactions', function (err, settings) {
                ...
                async.series({
                    ...
                    post: function (next) {
                        db.getObject('post:' + data.pid, (err, data) => {
                            next(null, data);
                        });
                    },
                    ...
                    else if (parseInt(results.post.uid, 10) === data.uid) {
                        //user cannot react to their own post
                        callback(new Error('You cannot react to your own post'));
                    }
                    else {
    ...
    function sendEvent(data, eventName, callback) {
        async.series({
            ...
            user: function (next) {
                user.getUsernamesByUids([data.uid], (err, data) => {
                    next(null, data[0]);
                });
            }
        }, function (err, results) {
            ...
            if (eventName === 'event:reactions.removePostReaction') {
                // notifications.rescind('plugin:reactions:' + data.pid);
                ...
                let text;
                if (parseInt(results.reactionCount, 10) === 1) {
                    text = `${results.user} reacted to your post`;
                }
                if (parseInt(results.reactionCount, 10) > 1) {
                    text = `${results.user} and ${parseInt(results.reactionCount, 10) - 1} other(s) reacted to your post`;
                }

                notifications.create({
                    nid: 'plugin:reactions:' + data.pid,
                    bodyShort: text,
                    path: '/post/' + data.pid,
                    from: data.uid
                    ...
                    if (results.reactionCount > 0) {
                        notifications.push(notification, results.post.uid, () => {
                            websockets.in('topic_' + data.tid).emit(eventName, {
                                pid: data.pid,
                                uid: data.uid,
                                reaction: data.reaction,
